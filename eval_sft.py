...
def main():
    ...
    ap.add_argument("--base_model_name", type=str, default="google/gemma-3-270m-it")
    ap.add_argument("--sft_model_dir", type=str, default="./gemma3-kansaiben-sft")
    ...
    ap.add_argument(
        "--prompts", type=str, default=None, help="カンマ区切りで明示指定（例: 'A,B,C'"
    )
    args = ap.parse_args()
    ...
    # tokenizer は SFT 側に保存がある場合はそちらを優先
    try:
    ...
    if hasattr(sft.config, "use_cache"):
        sft.config.use_cache = True
