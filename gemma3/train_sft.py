...
# 設定（SFT: フルファインチューニング版）
MODEL_NAME = "google/gemma-3-270m-it"
FINAL_MODEL_DIR = "./gemma3-kansaiben-sft"
...
def tokenize_supervised(
    ...
    for msgs in examples["messages"]:
        ...
        labels[:prompt_len] = [-100] * prompt_len  # プロンプト部分は損失無効
...
def main():
    ...
    print("Gemma3 Kansai-ben SFT（フルFT）開始")
    set_seed(SEED)
    ...
    print("ベースモデルをロード中...")
    model, attn_impl = load_base_model_with_best_attn(MODEL_NAME, dtype)

    # 学習時は use_cache をオフ（警告回避 & メモリ節約）
    if hasattr(model.config, "use_cache"):
        model.config.use_cache = False

    print_trainable_params(model)  # フルFTなので全層が学習対象
